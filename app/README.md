...
実機で使う ABI（`armeabi-v7a` / `arm64-v8a`）だけを入れています。既定では `x86` と `x86_64` も入り、**その分だけで約 31MB あります。**エミュレータ用しかないので落としています。

`eas.json` の `env` で Gradle のプロパティを上書きしているだけなので、**APK は 1 つのまま**です。ABI ごとの分割はしていません。分割すると入れる人が自分の端末に合うものを選ぶことになります。

`expo run:android` の開発ビルドには影響しません。
