...
SD カードを抜き差ししなくても、スマホから直接送れます。画像のほかに、その場で書いた文字や、項目を並べた名刺も送れます。

1. 一覧から `[WiFi]` を選ぶ
2. 画面に出た QR コードをスマホのカメラで読む（本体のアクセスポイントに繋がります）
3. ブラウザが自動でアップロード画面を開く（開かない場合は `http://192.168.4.1`）
4. **画像**を選ぶか、**テキスト**に文字を書くか、**名刺**の項目を埋めて送信すると、本体に表示されます

送る前にブラウザ側で画面の大きさに縮小するので、スマホで撮った大きな写真でもそのまま選べます。EXIF の向きも反映されます。

**テキストはブラウザで画像にしてから送ります。** 書いた文字はブラウザ側で画像に描き起こされ、画像と同じ経路で本体に届きます。縦横の大きさは指定でき、はじめは本体の画面と同じ値が入っています（`縦横を入れ替え` で横長にもできます）。
...
**名刺は項目を埋めるだけで作れます。** 決まった形に並べるので、文字寄せや大きさを決める必要はありません。

| 項目 | 中身 |
| --- | --- |
| 画像 | 顔写真やロゴ。任意 |
| タイトル | 名前など、いちばん大きく出す 1 行 |
| サブタイトル | 英字表記や肩書き。タイトルより小さく出す |
| SNS アカウント | `@name` など。さらに小さく出す |
| QR にする URL | ブラウザ側で QR コードにして載せる |

- **空にした項目は場所を取りません。**残った項目が詰まって真ん中に来ます。全部埋める必要はありません
- **枠が横長のときは、画像を左に置いて文字と QR を右に積みます。**縦長のときは上から順に積みます
- **QR は URL から作ります。**画像を用意する必要はありません。載せられるのは 213 バイト（日本語なら 71 文字）までで、超えるとその旨を出して送信を止めます
- **文字の大きさ**はテキストと同じように調整できます。既定は枠に収まる最大（自動）で、そこから 40% まで小さくできます。タイトル・サブタイトル・SNS アカウントは連動して小さくなります
...
WiFi に繋がない環境でも、専用のアプリから NFC で画像を送れます。画像のほかに、その場で書いた文字や名刺も送れます。

1. 一覧から `[NFC]` を選ぶ
2. アプリ（`app/`）で画像を選ぶか、`テキスト` に文字を書くか、`名刺` の項目を埋めて、送信を押す
3. iPhone を本体にかざしたまま待つ

**テキストと名刺の作り方は WiFi のときと同じです。** 枠の縦横、折り返し、文字寄せ、文字の大きさ、名刺の項目まで同じように指定できます。描くコードも本体の WiFi 画面と同じものを使っているので、どちらから送っても同じ見た目になります。
