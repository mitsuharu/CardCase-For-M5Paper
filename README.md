...
SD カードを抜き差ししなくても、スマホから直接送れます。画像のほかに、その場で書いた文字も送れます。
...
**テキストはブラウザで画像にしてから送ります。** 書いた文字はブラウザ側で画像に描き起こされ、画像と同じ経路で本体に届きます。縦横の大きさは指定でき、はじめは本体の画面と同じ値が入っています（`縦横を入れ替え` で横長にもできます）。文字の大きさは指定した枠に収まる最大を自動で選ぶので、文字数を変えても入り切らなくなりません。
...
## NFC で画像を送る（M5PaperMono のみ）

WiFi に繋がない環境でも、専用のアプリから NFC で画像を送れます。

1. 一覧から `[NFC]` を選ぶ
2. アプリ（`app/`）で画像を選び、送信を押す
3. iPhone を本体にかざしたまま待つ

NFC は数 cm しか届かず途中で切れやすいので、切れても**受け取り済みの位置から自動で続き**ます。かざしたままにしていれば送り切れます。

**ブラウザからは送れません。** iOS には NFC の API が無く、Android の Web NFC も NDEF しか扱えないためです。手軽さを取るなら WiFi のほうが向いています。

SD カードがあれば `NFC.jpg` として保存し、次回から一覧に並びます。無い場合は表示するだけです。

送る側のアプリは `app/` にあります。ビルドと配布は [app/README.md](app/README.md) を参照してください。

自分で実装したい場合は [NFC 転送プロトコル](docs/nfc-protocol.md) を参照してください。バイト列の実例と検査用の値を載せています。
