...
WiFi に繋がない環境でも、専用のアプリから NFC で画像を送れます。画像のほかに、その場で書いた文字も送れます。

1. 一覧から `[NFC]` を選ぶ
2. アプリ（`app/`）で画像を選ぶか、`テキスト` に文字を書いて、送信を押す
3. iPhone を本体にかざしたまま待つ

**テキストの調整は WiFi のときと同じです。** 枠の縦横、折り返し、文字寄せ、文字の大きさを指定できます。描くコードも本体の WiFi 画面と同じものを使っているので、どちらから送っても同じ見た目になります。
