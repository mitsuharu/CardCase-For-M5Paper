...
- **語の途中では折り返さない**。まず割らずに収まる大きさを探し、1 語が枠の幅に入らないときだけ文字単位に割ることを許して探し直す。先に割ると、小さくすれば 1 行に入るアドレスが途中で切れる
...
- **大きさの指定は自動を基準にした割合にする**（40〜100%）。px で直に指定させると、枠を変えたときに入り切らなくなる。自動が上限なので、それより大きくはできない。割合で小さくするときは **12px を下限にする** — 電子ペーパーでは画数の多い漢字が潰れて読めなくなるため。文字数が多くて自動の時点で 12px を切る場合は、そちらを優先する
