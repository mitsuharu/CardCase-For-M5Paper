...
`lib/WebTransfer` が本体をアクセスポイントにして、スマホのブラウザから画像を受け取る。ブラウザで書いた文字も、画像にしてから同じ経路で受け取る。
...
アップロード画面には「画像」と「テキスト」がある。テキストは canvas に描いて PNG にし、画像と同じ `/upload` へ送る。**本体側に文字を送るための経路は無い。**

- **本体にフォントを持たせない**。日本語のフォントはフラッシュを大きく食う上に、機種ごとの画面に合わせた折り返しと大きさの調整も要る。スマホのフォントで描けばどちらも要らない
- **大きさは指定した枠に収まる最大を二分探索で決める**。枠は機種の画面と同じとは限らず（利用者が変えられる）、文字数も毎回違うので、固定の値では入り切らないか小さすぎるかのどちらかになる。本体側の「文字は幅に収まる大きさを実行時に選ぶ」と同じ考え方
- **折り返しは 3 段階で、緩い順に試す**。(0) 書いたとおりの行のまま (1) 折り返すが語の途中では切らない (2) 語の途中でも切る。段が緩いほど字を大きくできるので、**厳しい側から順に採る**
  - **書いた人の改行を優先する。**日本語は語の切れ目が無いので、先に折り返しを許すと「山田 太」「郎」と割ってでも字を大きくしてしまう（540x960 で 64px → 128px）。改行は指定として扱う
  - **ただし読めなくなるなら折り返しに任せる。**長い 1 行をそのまま入れようとすると極端に小さくなる（60 字を 1 行だと 7px、折り返せば 64px）。境目は `READABLE`（12px）で、割合の下限と同じ値を使っている
  - **語の途中で切るのは最後の手段。**先に割ると、小さくすれば 1 行に入るアドレスが途中で切れる
  - **利用者が「折り返さない」を選べる。**選ばれたら小さくなっても (0) のままにする。折り返されると意味が変わるもの（表の見出しなど）のための逃げ道
  - **どれでも入らないときは何も描かない。**呼び出し側が「入りません」と出して送信を止める。白いままの画像を送らせない
- **太字で描く**。電子ペーパーは階調が粗く、細い線が飛ぶ
- 枠の既定値は機種の画面の大きさにしてある。**そこから外れると本体側で拡大・縮小が入る**（`drawPng` の自動フィット）。既定のままがいちばんきれいに出る
- **打っている間は描き直さない**。1 文字ごとに二分探索と PNG の生成が走ると、古いスマホでは入力が詰まる。手が止まってからにする
- **大きさの指定は自動を基準にした割合にする**（40〜100%）。px で直に指定させると、枠を変えたときに入り切らなくなる。自動が上限なので、それより大きくはできない。割合で小さくするときは **`READABLE`（12px）を下限にする** — 電子ペーパーでは画数の多い漢字が潰れて読めなくなるため。文字数が多くて自動の時点でそれを切る場合は、収めるほうを優先する
- **寄せは左右だけにする**。上下は常に中央。端に寄せるほどベゼルに近づいて読みにくくなるだけで、名刺の見え方としても得るものが無い

**テキストは接続画面の制約を受けない。**ファイル選択が絡まないので、Android の接続画面からでもそのまま送れる。案内にもそう書いてある。
...
手動実行もできる。ただし**バージョンを入れないと GitHub Release は作られない**（タグが無いので作りようがない）。焼いて DeployGate に上げるところまでで止まる。

### アプリとファームウェアのリリースは分ける

まとめない。ファームウェアは 1 日に何度も出る（`0.0.1` から `0.0.4` まで同じ日に出した）一方、**EAS の無料枠は月 15 回**しかない。一緒に回すと、アプリを 1 行も触っていないのに枠が消える。低優先度キューで 10〜20 分待つため、ファームウェアのリリースがアプリのビルドを待つことにもなる。

**本体とアプリの互換性は、タグではなくプロトコルの版で守る。**`lib/NfcTransfer/Protocol/Protocol.h` の `kVersion` と `app/src/protocol.ts` の `VERSION` が食い違えばフレームを弾く。タグを揃えても、本体だけ・アプリだけ更新する人は必ず出るので、結局こちらで守るしかない。
