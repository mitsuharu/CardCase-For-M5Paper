...
`textRender.ts` の中身はブラウザで動く JavaScript なので、node からは canvas を用意できない。`test/textRender.test.ts` と `test/cardRender.test.ts` は目印で挟んだところを取り出して、文字の幅を返すだけの偽物を渡して動かしている。折り返しと大きさの決め方、名刺の並べ方、QR の升目はこれで確かめられる。

**QR は決まった URL に対する升目を試験に置いてある。**読み取り機で復号できることを確かめた並びなので、誤り訂正やマスクの選び方を変えるとここで落ちる。
...
アップロード画面には「画像」「テキスト」「名刺」がある。テキストと名刺は canvas に描いて PNG にし、画像と同じ `/upload` へ送る。**本体側に文字を送るための経路は無い。**
...
**テキストは接続画面の制約を受けない。**ファイル選択が絡まないので、Android の接続画面からでもそのまま送れる。案内にもそう書いてある。名刺も、画像を載せなければ同じく送れる。

### 名刺は項目を並べて作る

「テキスト」が書いた文字をそのまま画像にするのに対し、「名刺」は決まった項目（画像・タイトル・サブタイトル・SNS アカウント・QR にする URL）を受け取って並べる。

- **組み方は決め打ちにして、寄せや大きさは選ばせない。**名刺としての収まりは並べ方で決まっていて、触れるようにしても読みにくい組み方が増えるだけになる
- **空の項目は場所を取らない。**残った項目を詰めて中央に置く。全部埋める前提にすると、名前だけ載せたいときに余白が間延びする
- **枠が横長で画像があるときは、画像を左に置いて文字と QR を右に積む。**1 列に積むと画像が潰れ、左右が空いたままになる。縦長のときは上から順に 1 列で積む
...
- **タイトルは折り返さずに入る大きさを先に採る。**テキストの側と同じ理由で、折り返しを先に許すと「山田」「太郎」と名前を割ってでも字を大きくしてしまう。読めない大きさ（`READABLE`）まで落ちるときだけ折り返しに任せる
...
- **長すぎる URL は描く前に弾く。**`qrFits` で先に見て送信を止める。黙って QR の無い名刺を作らない
- **升目は整数の大きさで描く。**電子ペーパーは階調が粗く、半端な大きさで描くと升目の境が濁って読めない
- **周りの余白（クワイエットゾーン 4 升）も自分で持つ。**これが無いと、読み取り機が符号の端を見つけられない
- **マスクは 8 通り試して、規格どおりの点数がいちばん低いものを採る。**どれを選んでも復号はできるが、偏った並びは読み取りにくい
- **詰め物は必ず `0xec` から始めて交互に置く。**ここがずれると誤り訂正語まで変わり、出来上がった QR は読めなくなる。並べ方の間違いと違って絵は正しく見えるので、気づきにくい
