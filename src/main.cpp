 ...
 *
 * 待ち受け自体は起動時から動いているが、受け取るのは [NFC] を
 * 選んでいる間だけ。一覧を見ている最中に届いても困るため。
 * それ以外では NOT_READY を返して断っている。
 */
...
void buildMenu()
{
  ...
  //
  // M5PaperColor では特に効く。一覧を 1 行動かすたびに全面更新が入り、
  // 十数秒かかる。末尾に置くと画像の枚数だけ待たされる。
  // 先頭なら 1 手で届き、スマホから送れば本体の更新は表示の 1 回で済む。
